...
function hsToggleCollapsing(header) {
  if (header.parent().hasClass("hsCollapsed")) hsExpand(header);
  else if (header.parent().hasClass("hsExpanded")) hsCollapse(header);
}
...
function hsInit() {
  for (var i = 3; i <= 5; i++) {
    $("#content .outline-" + i).each(function () {
      ...
      if (header.siblings().length > 0) {
        ...
        if (HS_ALWAYS_DISPLAY_ICON == true) {
          header.append('<span class="ellipsis"> ' + HS_ICON_EMPTY + "</span>");
        }
  ...
  $(".buttons").append(
    $("<span>" + HS_SHOW_ALL_TEXT + "</span>")
      .addClass("hsButton")
      .click(hsExpandAll)
  );
  $(".buttons").append(
    $("<span>" + HS_HIDE_ALL_TEXT + "</span>")
      .addClass("hsButton")
      .click(hsCollapseAll)
  );
}
...
function hsReviewTaskNext(backward) {
  ...
  if ($(".hsReviewing").length == 0) {
    // reviewing is starting
...
function hsAddReviewPanels() {
  $(".outline-2").each(function (e) {
    ...
    if (nbItems > 0) {
      $(this).prepend(
        '<div class="hsReviewPanel hsUnselectable" onclick="hsReviewTaskNext()">' +
...
function hsAddReviewingPanels() {
  $(".outline-2").each(function (e) {
    ...
    if (nbItems > 0) {
      $(this).prepend(
        '<div class="hsReviewingPanel hsUnselectable">Reviewing task <span class="hsReviewItem">1</span> / ' +
          nbItems +
          "<br>" +
          "(out of " +
          nbReviewTotalTasks +
      ...
      $(this).prepend(
        '<div class="hsReviewingPanel hsUnselectable">No task to review<br>' +
          "(out of " +
          nbReviewTotalTasks +
          " tasks)<br>" +
          "Shortcuts: " +
